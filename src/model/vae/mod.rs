...
use tch::{nn::{self, Module, OptimizerConfig}, Kind, Tensor};
...
impl Vae {
    fn new(vs: &nn::Path) -> Self {
            Vae { fc1: nn::linear(vs / "fc1", 784, 400, Default::default()),
    ...
    fn encode(&self, input: &Tensor) -> (Tensor, Tensor) {
        let h1 = input.apply(&self.fc1).relu();
        (self.fc21.forward(&h1), self.fc22.forward(&h1))
    }

    fn decode(&self, z: &Tensor) -> Tensor {
        z.apply(&self.fc3).relu().apply(&self.fc4).sigmoid()
    }

    fn forward(&self, input: &Tensor) -> (Tensor, Tensor, Tensor) {
        let (mu, logvar) = self.encode(&input.view([-1, 784]));
        let std = (&logvar * 0.5).exp();
        let eps = std.rand_like();
        (self.decode(&(&mu + eps * std)), mu, logvar)
    }
}

fn loss(recon_x: &Tensor, x: &Tensor, mu: &Tensor, logvar: &Tensor) -> Tensor {
    let bce = recon_x.binary_cross_entropy::<Tensor>(&x.view([-1, 784]), None, tch::Reduction::Sum);
...
fn image_matrix(imgs: &Tensor, sz: i64) -> Result<Tensor> {
    ...
    for i in 0..sz {
        ys.push(Tensor::cat(&(0..sz).map(|j| imgs.narrow(0, 4 * i, 1)).collect::<Vec<_>>(), 2))
    }

    Ok(Tensor::cat(&ys, 3).squeeze_dim(0))
}

pub fn run(lr: f64, epochs: i32, batch_size: i32) -> Result<()> {
    ...
    for epoch in 1..epochs+1 {
        ...
        for (bimages, _) in m.train_iter(batch_size as i64).shuffle().to_device(vs.device()) {
        ...
        let s = Tensor::randn([64, 20], (tch::Kind::Float, tch::Device::Cpu)).to(device);
